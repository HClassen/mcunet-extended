...
class SampleManager():
    ...
    def _append_measurement(
        self, fn: Callable[[Model, float, int], tuple[Any, ...]], i: int, j: int
    ) -> None:
        model = self._models[i][j]
        width_mult = self._spaces[i].width_mult
        resolution = self._spaces[i].resolution

        self._measurements[i].append(fn(model, width_mult, resolution))
    ...
    def measure(
        ...
        if idx0 == len(self._measurements):
    ...
    def from_dict(
        ...
        manager._measurements = config["measurements"]
        manager._index = config["index"]
...
def _test_and_add_to_top(
    ...
    *,
    ...
    for candidate in population:
        ...
        accuracy = candidate.run_valid(ds, batch_size, batches=batches, device=device)
