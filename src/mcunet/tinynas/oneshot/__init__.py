...
def _make_caption(text: str, line_length: int, divider: str) -> str:
...
class SuperNet(MobileSkeletonNet):
    ...
    def run_warm_up(
        ...
        print(_make_caption("Warm Up", 70, " "))
        for i, max_model in enumerate(space.max_models()):
            ...
            for j in range(epochs):
                print(_make_caption(f"Epoch {j + 1}/{epochs}({i + 1})", 70, "-"))

                epoch_start = time.time()

                for k, (images, labels) in enumerate(dl):
                    print(f"epoch={j + 1}({i + 1}), batch={k + 1:0{len(str(batches))}}/{batches}", end="")
            ...
            print(_make_caption(f"Set Parameters({i + 1})", 70, "-"))
    ...
    def run_train(
        ...
        print(_make_caption("Training", 70, " "))
        for i in range(epochs):
            print(_make_caption(f"Epoch {i + 1}/{epochs}", 70, "-"))
