...
class SuperNet(MobileSkeletonNet):
    ...
    def __init__(
        ...
        pool = build_pool()
        classifier = build_classifier(n_classes, last_out, dropout)
...
def _copy_conv2d(
    ...
    new[1].weight = nn.Parameter(old[1].weight[:c_out])
    new[1].bias = nn.Parameter(old[1].bias[:c_out])
    new[1].running_mean = old[1].running_mean[:c_out]
    new[1].running_var = old[1].running_var[:c_out]
...
def _reduce_channels(op: BaseOp, in_channels: int, out_channels: int) -> BaseOp:
    ...
    if isinstance(op, Conv2dOp):
        ...
        reduced.layers[i].weight = \
            nn.Parameter(op.layers[i].weight[:, :in_channels])
        reduced.layers[i + 1] = op.layers[i + 1]
        ...
        i += 2 if se_ratio > 0.0 else 1
        reduced.layers[i].weight = nn.Parameter(op.layers[i].weight[:, :hidden])
        reduced.layers[i + 1] = op.layers[i + 1]
...
def crossover(p1: OneShotNet, p2: OneShotNet) -> OneShotNet:
    ...
    ``s`` from ``p1`` and merge them with layers ``s + 1`` to ``6`` from ``p2``
    to create the offspring model.
    ...
    Returns:
        OneShotNet:
                The offspring model.
    """
    split = torch.randint(0, 7, [1]).item()
    ...
    if 0 < split < 6:
        ...
        if p1_out_channels < p2_in_channels:
            ...
            n_layers = p1_split_lengths[-1]
            for i in range(n_layers - 1):
                p1_blocks[-1 - i] = _reduce_channels(
                    p1_blocks[-1 - i],
                    p2_in_channels,
                    p2_in_channels
                )

            p1_blocks[-1 - n_layers] = _reduce_channels(
                p1_blocks[-1 - n_layers],
                p1_blocks[-1 - n_layers].in_channels,
                p2_in_channels
            )

    blocks = p1_blocks + p2_blocks
    last = deepcopy(p1.last) if split == 6 else deepcopy(p2.last)
