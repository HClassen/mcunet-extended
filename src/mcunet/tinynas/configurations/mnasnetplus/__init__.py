...
class CommonWarmUpCtx(WarmUpCtx):
    ...
    def post(self, supernet: MobileSkeletonNet) -> None:
        ...
        with torch.no_grad():
            ...
            reordered, importance = l1_sort(self._last_parameters[0])

            sharer.conv2d_weight.copy_(reordered)
            for k, channel in enumerate(importance):
                sharer.norm_weight[k] = self._last_parameters[1][channel]
                sharer.norm_bias[k] = self._last_parameters[2][channel]
