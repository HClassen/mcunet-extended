...
__all__ = [
    "BaseChoiceOp", "Conv2dChoiceOp", "DWConv2dChoiceOp", "BDWRConv2dChoiceOp",
    "BaseBlock", "Conv2dBlock", "DWConv2dBlock", "BDWRConv2dBlock",
    "SuperBlock",
    "LastConvChoiceOp"
]
...
class BaseChoiceOp(ABC, nn.Module):
            ...
            for j in range_out_channels:
                for e in range_expansion_ratios:
                    for k, se, skip in _permutations():
                        ...
                        choice[Layer.CONV2D][0].weight = None
    ...
    def _share_weight(self, choice: BaseOp) -> None:
        ...
        conv2d: nn.Conv2d = choice[Layer.CONV2D][0]
    ...
    def set(
        ...
        for choice in self.choices:
            ...
            if not (
                ...
                choice[Layer.CONV2D][0].kernel_size == (kernel_size, kernel_size)
            ):
                continue
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        self.active[Layer.CONV2D][0].weight = None
...
class BDWRConv2dChoiceOp(BaseChoiceOp):
    """
    A concrete instance of a ``BaseChoiceOp`` which contains ``BDWRConv2dOp`` of
    the MnasNet search space.
    """
    def _build_op(
        self,
...
class BDWRConv2dBlock(BaseBlock):
    """
    A concrete instance of a ``BaseBlock`` which contains ``depth`` many layers
    of ``BDWRConv2dChoiceOp``.
    """
    def _build_choice_op(
        self,
        range_in_channels: Iterable[int],
...
class SuperBlock(nn.Module):
    ...
    def __init__(
        ...
        if not (
            conv2d.in_channels == dwconv2d.in_channels == bdwrconv2d.in_channels
            and
            conv2d.out_channels == dwconv2d.out_channels == bdwrconv2d.out_channels
        ):
